...
- **The signal is `in` against `crate`** in reservoarr's `delaybuf.log`: what the provider
...
- **The reference evening** is 7-8 September 2026 (UTC), kept in
  `tests/fixtures/delaybuf-2026-09-07-08.log.gz`: 44 triggers at Stable after 0, 58 at 180, all
  on 202096, 94281, 202121 and 202099. Retune only with that test and a replay of a fresh log.
  Replay counts triggers, not switches.
